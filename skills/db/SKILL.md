...
- `/topia db` — manual invocation when schema changes are planned
- Called by `build` (L1): schema change detected in diff
...
- `build` (L1): schema change detected in diff
...
Update or create `.topia/schema-changelog.md` with a human-readable entry:
...
| Schema changelog not updated after migration | LOW | Step 7 runs always — log INFO if skipped due to no .topia/ directory |
...
- Schema changelog updated in .topia/schema-changelog.md
...
| Schema changelog entry | Markdown | `.topia/schema-changelog.md` |
