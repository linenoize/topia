...
- Manual: `/topia brainstorm vision <product area>`
...
- Manual: `/topia brainstorm design-it-twice <module>`
...
- `build` Phase 4: Approach Pivot Gate fires (3 debug-fix loops exhausted + re-plan still fails)
...
- Manual: `/topia brainstorm rescue <what failed and why>`
...
- Called by `build` when multiple valid approaches exist for a feature (Discovery Mode)
- Called by `build` Approach Pivot Gate when current approach fundamentally fails (Rescue Mode)
...
- `/topia brainstorm <topic>` — manual brainstorming (Discovery Mode)
- `/topia brainstorm rescue <context>` — manual rescue (Rescue Mode)
...
- `build` (L1): when multiple valid approaches exist for a feature (Discovery Mode)
- `build` (L1): Approach Pivot Gate — current approach failed, need category-diverse alternatives (Rescue Mode)
...
- User: `/topia brainstorm <topic>` direct invocation (Discovery Mode)
- User: `/topia brainstorm rescue <context>` manual rescue (Rescue Mode)
- `idea` (L2): when multiple requirement approaches exist
