...
- Called by `build` post-Phase 7 to update docs after feature implementation
- Called by `launch` pre-deploy to ensure docs are current
- `/topia docs init` — first-time documentation generation
- `/topia docs update` — sync docs with recent code changes
- `/topia docs api` — generate API documentation
- `/topia docs changelog` — auto-generate changelog from git history
...
Read `git diff` since last docs update (tracked via git log on doc files or `.topia/docs-sync.json`).
