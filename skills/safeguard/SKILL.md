...
```typescript
// @legacy — Topia-safeguard [date] — do not refactor without characterization tests passing
```
...
git tag Topia-safeguard-<module>
```
...
- **Rollback Tag**: Topia-safeguard-[module]
- **Config Frozen**: [list of files in .topia/]
...
| No git tag created before declaring safeguard complete | MEDIUM | Tag `Topia-safeguard-<module>` must exist before surgery begins |
...
- Config files frozen to .topia/
- Git tag `Topia-safeguard-<module>` created
...
| Frozen config snapshot | Copies of config files | `.topia/*.frozen.*` |
| Git rollback tag | Git tag | `Topia-safeguard-<module>` |
