...
- Called by `build` Phase 0.5 — before planning, after resume check (first run in a new project only)
...
- `/topia env-check` — manual environment validation
- Auto-trigger: when `npm install`, `pip install`, or similar fails during build
...
- `build` (L1): Phase 0.5 — first run detection (no `.topia/` directory exists)
- `scaffold` (L1): post-bootstrap environment validation
- `onboard` (L2): developer onboarding verification
- User: `/topia env-check` direct invocation
