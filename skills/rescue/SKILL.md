...
- `/topia rescue` — manual invocation on legacy project
...
- User: `/topia rescue` direct invocation
...
Bash: git tag Topia-rescue-safety-net
```
...
If approaching context limit (50+ tool calls or user signals fatigue):
  ...
  → Report: "Session limit reached. Rescue state saved. Resume with /topia rescue to continue."
...
`/topia rescue status` — reads RESCUE-STATE.md via `journal` and presents:
...
Rollback point: git tag Topia-rescue-baseline (set in Phase 0)
```
...
| Scope | Access | Files |
|-------|--------|-------|
| **Owns** (read + write) | `RESCUE-STATE.md`, `.topia/rescue-*.md`, git tags (`Topia-rescue-*`), refactored source files (one module per session) |
| **Reads** (never writes) | `CLAUDE.md`, autopsy reports, safeguard test files, `.topia/contract.md` |
...
| **No rollback point** — surgery without a tagged baseline | If surgery goes wrong, no safe state to return to | `git tag Topia-rescue-baseline` before ANY code changes |
...
|---|---|---|
| Starting surgery before safety net committed and tagged | CRITICAL | HARD-GATE: `Topia-rescue-safety-net` git tag must exist before Phase 2 |
