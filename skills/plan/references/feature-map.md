...
`.topia/features.md` — auto-created on first plan run, updated on every subsequent run.
...
When `.topia/features.md` does not exist, reverse-engineer from codebase:
...
5. **Write `.topia/features.md`** with detected features
...
When `.topia/features.md` exists, update incrementally:
