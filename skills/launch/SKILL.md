...
- `/topia launch` — manual invocation
...
- User: `/topia launch` direct invocation
...
RECOMMENDED ARTIFACTS (warn if missing, don't block):
  Design system:      Glob .topia/design-system.md — if frontend project
...
- Design system: ✅ .topia/design-system.md | ⚠️ Not found (run /topia design first for UI projects)
```
...
```
REQUIRED SUB-SKILL: topia:browser-pilot
...
```
REQUIRED SUB-SKILL: topia:watchdog
...
```
REQUIRED SUB-SKILL: topia:marketing
...
If user requested video content:
  REQUIRED SUB-SKILL: topia:video-creator
...
| Marketing assets (copy, social, SEO) | Markdown (inline) | Generated by `topia:marketing`, presented in Phase 5 |
