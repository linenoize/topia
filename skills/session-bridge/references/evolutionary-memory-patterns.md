...
```yaml
# .topia/memory/decisions.md entry
---
type: decision
id: dec-20260315-001
...
```yaml
# .topia/memory/findings.md entry
---
type: finding
id: fnd-20260316-003
...
```yaml
# .topia/memory/failures.md entry
---
type: failure
...
4. PTopia noise: Findings with `actionable: false` and `significance: low` older than 14 days — delete
...
The original `.topia/decisions.md` stays as a human-readable narrative log. The new `.topia/memory/` files are structured for agent querying.

### Integration points in session-bridge flow

**Save Mode — after Step 2 (decisions):**
- For each architectural decision, also write a typed Decision node to `.topia/memory/decisions.md`
...
- Inject summary into the Step 3 context report:
  ...
  - Unconsumed findings: [N] (see .topia/memory/findings.md)
...
signals:
  ...
  listens:
    - phase.complete:
        action: Run end-of-session consolidation pass (merge duplicates, pTopia noise)
    - task.failed:
