...
This file defines the `.continue-here.md` format used when build must pause mid-phase
(context limit, user break, session end before phase completes).

## When to Use

When build cannot complete the current phase in one session:
...
1. Create `.topia/.continue-here.md` with the format above
2. Create a WIP commit: `wip: build phase N paused at task M`
3. Stop execution
