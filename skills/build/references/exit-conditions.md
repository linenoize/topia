...
Every build invocation inside `team` or autonomous workflows MUST respect these exit conditions.
...
MAX_REPLAN:        1 re-plan per build session (Phase 4 re-plan check)
MAX_PIVOT:         1 approach pivot per build session (Approach Pivot Gate)
...
- Execution stops
- User must make a decision before build can resume
