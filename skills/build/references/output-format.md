...
Full format for the Cook Report emitted at the end of every build session.
Also defines the NEXUS Deliverables table format used when build is invoked by `team`.
...
Every Cook Report MUST end with a `chain_metadata` YAML block. This enables downstream skills to consume build's output data without parsing prose. See `docs/references/chain-metadata.md` for the full contract.
...
**When NOT to emit:** When build is invoked as a sub-step of `autopilot` or `team` — the orchestrator emits its own chain_metadata at the end.
