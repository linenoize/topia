...
- User: `/topia deploy` direct invocation
...
- L4 extension packs: domain-specific deploy patterns when context matches (e.g., @Topia/devops for infrastructure)
...
| 5 | Rollback plan documented | `.topia/deploy/rollback-<version>.md` exists | WARN if missing |
...
**Rollback Plan Template** (`.topia/deploy/rollback-<version>.md`):
...
| Rollback plan document | Markdown | `.topia/deploy/rollback-<version>.md` |
