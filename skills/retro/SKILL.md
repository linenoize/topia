...
Engineering retrospective engine. Analyzes git history, work patterns, and code quality signals to produce actionable retrospectives with per-person breakdowns, shipping streaks, and concrete improvement habits. Fills a gap in the Topia ecosystem — build builds, review checks, but nothing reflects on HOW the team works.

<HARD-GATE>
Retro is READ-ONLY. It analyzes and reports — it does NOT modify code, create PRs, or change any files except its own output artifacts (.topia/retros/).
Retro is ENCOURAGING but CANDID. Every critique is anchored in specific commits, not vague impressions.
</HARD-GATE>

## Triggers

- `/topia retro` — default 7-day retrospective
- `/topia retro 24h` — daily standup review
- `/topia retro 14d` — sprint retro (2 weeks)
- `/topia retro 30d` — monthly review
- `/topia retro compare` — current vs previous period side-by-side
...
- `build` (L1): optional — after completing a multi-phase feature, suggest retro
- `rescue` (L1): post-rescue retrospective
- `launch` (L1): post-launch retrospective
- User: `/topia retro` direct invocation
...
- `.topia/retros/` history: previous retro JSON for trend comparison
...
- `.topia/retros/` for prior retro history (if exists)
...
Read most recent `.topia/retros/*.json`. Compute deltas:
...
Write JSON snapshot to `.topia/retros/{YYYY-MM-DD}.json`:
...
Count total retro snapshots in `.topia/retros/` (each represents ~1 retro session). Trigger milestone analysis when count reaches:
...
2. **Load history**: Read ALL `.topia/retros/*.json` snapshots (not just the most recent)
...
6. **Save**: Write milestone report to `.topia/retros/{YYYY-MM-DD}-milestone-{name}.md`
...
- Each milestone triggers ONCE — check if `.topia/retros/*-milestone-{name}.md` already exists before generating
...
When invoked as `/topia retro compare`:
...
- [ ] Retro JSON saved to .topia/retros/ for trend tracking
...
| Retro JSON snapshot | JSON | `.topia/retros/{YYYY-MM-DD}.json` |
...
**Scope guardrail:** retro is read-only — it analyzes and reports. It does NOT modify code, create PRs, or change any files except its own output artifacts in `.topia/retros/`.
