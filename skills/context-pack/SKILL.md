...
- Called by `build`, `team`, `rescue` before spawning subagents
...
- `build` (L1): before Phase 2-5 subagent spawning
...
<MUST-READ path="references/agent-brief.md" trigger="when handing off to an AFK agent, an issue tracker queue, or any receiver that may execute hours/days later"/>

The standard packet (above) is for **immediate** sub-agent dispatch in the same session. When the handoff target is async — issue tracker, scheduled cron agent, or any receiver that may execute hours or days later — switch to the **agent brief** variant.
