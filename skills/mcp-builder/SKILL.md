...
- Called by `build` when MCP-related task detected (keywords: "MCP server", "MCP tool", "model context protocol")
- Called by `scaffold` when MCP Server template selected
- `/topia mcp-builder <description>` — manual invocation
- Auto-trigger: when project contains `mcp.json`, `@modelcontextprotocol/sdk`, or `fastmcp` in dependencies

## Calls (outbound)

- `idea` (L2): if user description is vague — elicit requirements for what tools/resources the server should expose
...
- `build` (L1): when MCP-related task detected
- `scaffold` (L1): MCP Server template in Phase 5
- User: `/topia mcp-builder` direct invocation
