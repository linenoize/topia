...
Specialized git operations utility. Handles semantic commits, PR descriptions, branch naming, and changelog generation with consistent conventions. Replaces scattered git logic across build Phase 7 and other skills with a single, convention-aware utility.

## Triggers

- Called by `build` Phase 7 for commit creation
...
- `/topia git commit` — manual semantic commit
- `/topia git pr` — manual PR generation
- `/topia git branch <description>` — generate branch name
- `/topia git changelog` — generate changelog from commits
- `/topia git release <version>` — create tagged release with changelog
...
- `build` (L1): Phase 7 — create semantic commit after implementation
...
- User: `/topia git` direct invocation
...
**Triggers:**
- `/topia git release <version>` — create release for specified version
...
If deploy reports customer email list available (via Topia-pay `/admin/emails`), flag for notification.
