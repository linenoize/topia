...
- `/topia logic-guardian` — manual invocation (scan project, generate/update manifest)
- Auto-trigger: when `build` or `fix` targets a file listed in `.topia/logic-manifest.json`
- Auto-trigger: when `surgeon` plans refactoring on logic-heavy modules
- Auto-trigger: when `.topia/logic-manifest.json` exists in project root
...
- `build` (L1): Phase 1.5 — when complex logic project detected, load manifest before planning
...
1. Use `Read` on `.topia/logic-manifest.json`
2. If file exists:
...
4. Obtain invariants (seeded by `onboard` — see onboard Step 5.4):
   ...
   - **Fallback**: if no signal was emitted this session, `Read` `.topia/INVARIANTS.md` directly and invoke `skills/session-bridge/scripts/load-invariants.js` to parse.
   ...
   - If the file is absent, log **WARN**: "INVARIANTS.md not found — run `Topia onboard` to seed baseline rules." Do not block.
...
### Phase 2 — Pre-Edit Gate (called by fix/surgeon/build)
...
5. **Cross-file invariant check** — for each rule loaded from `.topia/INVARIANTS.md` in Phase 0:
   - If the target file matches any rule's `WHERE` glob, surface the rule to the agent before the edit proceeds:
     ```
     INVARIANT (from .topia/INVARIANTS.md):
       WHAT:  <rule.what>
       WHY:   <rule.why>
     ```
...
4. Write manifest to `.topia/logic-manifest.json`
...
1. Update `.topia/logic-manifest.json` with:
...
### Manifest Schema (`.topia/logic-manifest.json`)
...
|------|----------|------------|
| PRE_EDIT | `.topia/logic-manifest.json` loaded + component spec displayed | BLOCK edit. Run Phase 0 + Phase 2 first. |
...
|---|---|---|
| Agent edits manifested file without loading manifest first | CRITICAL | Phase 2 gate: build/fix MUST call logic-guardian before editing manifested files |
...
- `.topia/logic-manifest.json` exists and passes Phase 1 validation (SYNCED)
...
|----------|--------|----------|
| Logic manifest | JSON | `.topia/logic-manifest.json` |
