...
- Called by `build` Phase 5 REVIEW — after implementation complete
- Called by `fix` for self-review on complex fixes
- `/topia review` — manual code review
...
- `build` (L1): Phase 5 REVIEW — post-implementation quality check
- `fix` (L2): complex fix requests self-review
- User: `/topia review` direct invocation
...
```
Pull project-specific overrides from `.topia/design-system.md` § Scale Minimums.
...
When `build` or `ship` checks review status: compare review commit hash with current HEAD. If different → WARN: "Review is stale — code changed since last review."
...
Append to Code Review Report when invoked standalone. Suppress when called as sub-skill inside an L1 orchestrator (build, team, etc.) — the orchestrator emits a consolidated block. See `docs/references/chain-metadata.md`.
