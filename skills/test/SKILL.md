...
TDD test writer for any workflow that touches code. Writes failing tests FIRST (red), then verifies they pass after implementation (green). Covers unit, integration, and e2e levels.
...
When invoked with `mode: "diff-aware"` or by `build` after implementation:
...
- `build` (L1): Phase 3 TEST — write tests first
...
- `build` (L1): test results (pass/fail/coverage) → build's Phase 5 quality gate evidence
...
When a plan with acceptance criteria exists (`.topia/features/<name>/plan.md` or phase file), every criterion MUST map to at least one test case.
...
Store evals in `.topia/evals/<feature>.md`:
...
1. Write eval definitions (capability + regression) → `.topia/evals/<feature>.md`
...
| Test Report | Markdown (inline) | Emitted to calling skill (build, fix, review) |

## Chain Metadata

Append to Test Report when invoked standalone. Suppress when called as sub-skill inside an L1 orchestrator (build, team, etc.) — the orchestrator emits a consolidated block. See `docs/references/chain-metadata.md`.
...
| Skipping the RED phase (not confirming FAIL) | HIGH | Run tests, confirm FAIL output before calling build/fix to implement |
...
- [ ] If idea requirements exist (REQ-xxx), every requirement has at least one test — check plan's Traceability Matrix
```
