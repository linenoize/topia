...
- `/topia llm-integration` — audit or improve LLM API usage
- `/topia rag-patterns` — build or audit RAG pipeline
- `/topia embedding-search` — implement or optimize semantic search
- `/topia fine-tuning-guide` — prepare and execute fine-tuning workflow
- `/topia ai-agents` — design and build stateful AI agents
- `/topia code-sandbox` — set up secure code execution for AI
- `/topia web-extraction` — build structured data extraction from web pages
- `/topia deep-research` — implement iterative AI research loops with convergence
- Called by `build` (L1) when AI/ML task detected
...
Calls → @Topia/devops (L4): ai-agents → edge-serverless for agent deployment (Workers, Lambda)
Calls → @Topia/backend (L4): ai-agents → API patterns for agent endpoints and WebSocket handlers
