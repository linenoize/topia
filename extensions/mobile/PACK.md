...
- `/topia react-native` — audit React Native architecture and performance
- `/topia flutter` — audit Flutter architecture and state management
- `/topia deep-linking` — set up or audit deep linking (Universal Links, App Links)
- `/topia push-notifications` — set up or audit push notification pipeline
- `/topia ota-updates` — set up or audit OTA update strategy
- `/topia app-store-prep` — prepare app store submission
- `/topia native-bridge` — audit or create native module bridges
- `/topia ios-build` — end-to-end iOS build, sign, archive, upload pipeline
- `/topia app-store-connect` — App Store Connect API operations (versions, screenshots, localization, IAPs)
- Called by `build` (L1) when mobile task detected
...
Calls → @Topia/ui (L4): design system tokens, palette, typography for mobile UI consistency
Calls → @Topia/backend (L4): API patterns for mobile backend integration (auth, push server)
Calls → @Topia/security (L4): code signing audit, API key management, certificate validation
Called By ← build (L1): when mobile task detected
Called By ← team (L1): when porting web to mobile
