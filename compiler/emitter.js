 ...
 *
 * @param {string} sourceSkillDir - e.g. skills/build/
 * @param {string} outputSkillDir - e.g. .codex/skills/Topia-build/
 ...
 * @param {string} outputScriptsDir - e.g. .cursor/rules/Topia-slides-scripts/
...
/**
 * Discover packs in the extensions directory.
 *
 * @param {string} extDir - path to extensions/ directory
 * @param {string[]} [enabledPacks] - list of enabled pack names (null = all)
 * @returns {Promise<Array<{path: string, dirName: string}>>} resolved pack entries
 */
export async function discoverPacks(extDir, enabledPacks = null) {
  const packs = [];
  if (!existsSync(extDir)) return packs;

  const entries = await readdir(extDir, { withFileTypes: true });

  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    if (enabledPacks && !enabledPacks.includes(entry.name) && !enabledPacks.includes(`@Topia/${entry.name}`)) {
      continue;
    }
    const packFile = path.join(extDir, entry.name, 'PACK.md');
    if (!existsSync(packFile)) continue;

    packs.push({ path: packFile, dirName: entry.name });
  }

  // Return sorted by dirName for deterministic output
  return packs.sort((a, b) => a.dirName.localeCompare(b.dirName));
}
 ...
 */ export async function buildAll({ TopiaRoot, outputRoot, adapter, disabledSkills = [], enabledPacks = null }) {
  ...
  // Discover extension packs
  const packEntries = await discoverPacks(extensionsDir, enabledPacks);
  ...
  for (const skillPath of skillPaths) {
    try {
      ...
      // Apply reference injections from packs
      let finalBody = body;
      ...
      if (adapter.useSkillDirectories) {
        // Directory-per-skill: .codex/skills/Topia-{name}/SKILL.md
  ...
  for (const packEntry of packEntries) {
    try {
      ...
      // Discover and append workflow templates
      ...
      // Normalize pack name for headers (ext-ui instead of @Topia/ui)
  ...
  if (adapter.name === 'openclaw' && adapter.generateManifest && adapter.generateEntryPoint) {
    ...
    // Write openclaw.plugin.json to parent of skills dir (.openclaw/Topia/)
...
const INTENT_KEYWORDS = {
  build: ['implement', 'build', 'create', 'add', 'feature', 'fix', 'code', 'write', 'make', 'develop'],
  ...
  idea: ['requirements', 'business analysis', 'user stories', 'stakeholder'],
