...
describe('resolveScriptsPath', () => {
  test('replaces {scripts_dir} with resolved path', () => {
    ...
    const result = resolveScriptsPath(body, '.cursor/rules/Topia-slides-scripts');
    assert.strictEqual(result, 'Run: `node .cursor/rules/Topia-slides-scripts/build-deck.js`');
  });
...
describe('adapter scriptsDir', () => {
  test('flat-file adapters return sibling dir pattern', () => {
    for (const name of ['cursor', 'windsurf', 'antigravity', 'generic', 'openclaw']) {
      ...
      assert.strictEqual(adapter.scriptsDir('slides'), 'Topia-slides-scripts', `${name} scriptsDir mismatch`);
    }
  });
...
describe('buildAll with scripts', () => {
  async function createTempSkillTree() {
    const tmp = path.join(tmpdir(), `Topia-scripts-test-${Date.now()}`);
  ...
  test('cursor: scripts copied to sibling dir, placeholder resolved', async () => {
    ...
    try {
      ...
      const scriptsOut = path.join(outputRoot, adapter.outputDir, 'Topia-test-slide-scripts');
      ...
      const mdc = await readFile(path.join(outputRoot, adapter.outputDir, 'Topia-test-slide.mdc'), 'utf-8');
      assert.ok(!mdc.includes('{scripts_dir}'), 'placeholder should be resolved');
      assert.ok(mdc.includes('.cursor/rules/Topia-test-slide-scripts/build-deck.js'), 'resolved path missing');

      // Plain skill: no scripts dir created
      assert.ok(
        !existsSync(path.join(outputRoot, adapter.outputDir, 'Topia-test-plain-scripts')),
  ...
  test('codex: scripts inside skill directory', async () => {
    ...
    try {
      ...
      // Scripts inside skill dir: .codex/skills/Topia-test-slide/scripts/
      const scriptsOut = path.join(outputRoot, adapter.outputDir, 'Topia-test-slide', 'scripts');
      ...
      const md = await readFile(path.join(outputRoot, adapter.outputDir, 'Topia-test-slide', 'SKILL.md'), 'utf-8');
      assert.ok(!md.includes('{scripts_dir}'), 'placeholder should be resolved');
      assert.ok(md.includes('.codex/skills/Topia-test-slide/scripts/build-deck.js'), 'resolved path missing');
    } finally {
      await rm(tmp, { recursive: true, force: true });
    }
  });
  ...
  test('skill without scripts: no error, no empty dirs', async () => {
    ...
    try {
      ...
      assert.ok(!existsSync(path.join(outputRoot, adapter.outputDir, 'Topia-test-plain-scripts')));
    } finally {
      await rm(tmp, { recursive: true, force: true });
    }
  });
  ...
  test('scripts content copied as-is (byte-for-byte)', async () => {
    ...
    try {
      ...
      const copied = await readFile(
        path.join(outputRoot, adapter.outputDir, 'Topia-test-slide-scripts', 'build-deck.js'),
        'utf-8',
      );
