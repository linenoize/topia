...
test('parse build SKILL.md', () => {
  const content = readFileSync(path.join(SKILLS_DIR, 'build/SKILL.md'), 'utf-8');
  const parsed = parseSkill(content, 'build/SKILL.md');
  ...
  assert.strictEqual(parsed.name, 'build');
...
describe('signals parsing', () => {
  ...
  test('real build skill has signals', () => {
    const content = readFileSync(path.join(SKILLS_DIR, 'build/SKILL.md'), 'utf-8');
    const parsed = parseSkill(content, 'build/SKILL.md');
    assert.ok(parsed.signals, 'build should have signals');
  ...
  test('top-level emit/listen (pack format)', () => {
    const content = [
      '---',
      'name: "feature-spec"',
      'pack: "@Topia/product"',
