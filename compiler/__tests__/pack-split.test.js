...
`;
  ...
  const parsed = parsePack(content, 'extensions/ui/PACK.md');

  assert.strictEqual(parsed.isSplit, false);
  assert.deepStrictEqual(parsed.skillManifest, []);
  assert.strictEqual(parsed.name, '@Topia/ui');
...
`;
  ...
  assert.strictEqual(parsed.name, '@Topia/backend');
});
...
describe('buildAll split pack auto-discovery', () => {
  async function createTempWithSplitPack() {
    const tmp = path.join(tmpdir(), `Topia-split-test-${Date.now()}`);
  ...
  test('auto-discovers skill files from skills/ subdir when manifest is empty', async () => {
    ...
    try {
      ...
      const packOutput = await readFile(path.join(outputRoot, adapter.outputDir, 'Topia-ext-test-pack.mdc'), 'utf-8');
