...
function parseFrontmatter(content) {
  ...
  for (const line of raw.split('\n')) {
    ...
    if (kvMatch) {
      ...
      // Comma-separated list fields at top level too
      if (COMMA_LIST_FIELDS.has(kvMatch[1])) {
...
export function parseSkill(content, filePath = '') {
  ...
  // Extract signals — emit/listen arrays from metadata or top-level
...
/**
 * Parse organization config from .topia/org/org.md
