...
try {
  const result = spawnSync('npx', ['prettier', '--version'], { cwd: dir, encoding: 'utf-8', stdio: 'pipe', timeout: 5000 });
  if (result.status === 0) {
    hasPrettier = true;
  }
} catch {
...
try {
  ...
  console.log(`[Topia auto-format] Prettier failed on ${path.basename(filePath)}: ${e.message.split('\n')[0]}`);
}
