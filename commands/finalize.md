---
name: finalize
description: One-shot finalize-install. Run inside Claude Code after `/plugin install Topia@protopia` to enable the optional pieces (system-wide dispatch hooks, agora-code persistent memory, project .gitignore) without leaving the chat. Idempotent — safe to re-run.
---

# /topia finalize

The marketplace install (`/plugin install Topia@protopia`) is already complete for the core experience: all 66 skills, all 64 subagents, the 11 plugin hooks (session-start, secrets-scan, quarantine, metrics-collector, …), and file-based `.topia/` memory all work the moment the plugin is enabled.
...
```bash
TOPIA_ROOT="$(ls -dt ~/.claude/plugins/cache/protopia/Topia/* 2>/dev/null | head -1)"
...
If neither path resolves, stop and tell the user: "Plugin cache not found. Run `/plugin install Topia@protopia` first, or clone the repo and re-run this command from inside the clone."
...
- It does **not** install the plugin itself — that's `/plugin install Topia@protopia`.
